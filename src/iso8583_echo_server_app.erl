...
load_interfaces(Interfaces)->
    lists:map(fun(Interface)-> 
			...
			server_address := _Server_Address_interface,spec_path := Spec_path_interface} = Interface,
		      Specification = iso8583_erl:load_specification(code:priv_dir(iso8583_echo_server)++"/"++Spec_path_interface),
		      {ok, _} = ranch:start_listener(Name_interface,ranch_tcp,#{socket_opts => [{port, Port_interface}],max_connections =>Limit_interface},iso8583_echo_server_sock_serv, [Byte_header_size_interface,Specification])		
	      end,Interfaces).
...
test()->
    ...
    Iso_Response = iso8583_erl:pack(Map_send_list,Specification),
    ...
    case gen_tcp:connect(Host, Port, [list, {active, once}]) of
	    ...
	    process_message([],Bheader,Specification),
...
process_message(State_iso,Bheader,Specification)->
    receive
	{tcp,_Sender,Message_Echo}->
	    State_new = lists:flatten([State_iso,Message_Echo]), 
	    case length(State_new) of 
		Size when Size < Bheader ->
		    process_message(State_new,Bheader,Specification);
		_  ->
			...
			SizeafterHead when Len =:= SizeafterHead ->
			    ...
			    io:format("~n message echoed back is ~p~n", [Map_iso]);
			SizeafterHead when Len < SizeafterHead ->
			    process_message(State_new,Bheader,Specification)
		    end
	    end
...
get_date_send_data()->
    ...
    erlang:list_to_binary([Yr_send,Mn_send,Dy_send,Hr_send,Min_send,Se_send]).
